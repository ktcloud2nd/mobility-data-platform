...
1. infra/edge/vehicle_simulator.py에서 100대 규모 차량 상태를 가상 생성합니다.
2. 속도, 위치, 연료량, 엔진 상태, 이벤트 타입을 Kafka raw_topic으로 전송합니다.
3. Azure Consumer VM에서 Kafka Consumer가 메시지를 소비해 정제 데이터와 이상 탐지 이벤트를 구분합니다.
4. Kafka Connect Container가 Sink Connector를 통해 데이터를 목적별로 분기 적재합니다.
5. 원천 데이터는 Azure Data Lake Storage로 적재됩니다.
6. 정제 데이터는 AWS RDS PostgreSQL의 vehicle_stats 테이블로 적재됩니다.
7. 이상 탐지 데이터는 AWS RDS PostgreSQL의 vehicle_anomaly_alerts 테이블로 적재됩니다.
8. 이상 이벤트는 별도 웹훅을 통해 AWS Lambda로 전달되고, 최종적으로 Slack 알림으로 이어집니다.
